...
class main_form(QMainWindow, Ui_MainWindow):
    ...
    def open_msg(self):
        ...
        self.tableview_show(file, self.data)
        #对数据进行分析，画图
        data_analyzer = analyzer.Data_analyzer(self.data.student_list)
        
    def tableview_show(self, file, data):
        # 设置tableview里的item
        self.model = QStandardItemModel(len(data.student_list), 3)
        self.model.setHorizontalHeaderLabels(['学号', '姓名', '成绩'])
        # model可以理解为一种数据集
        for row in range(0, len(data.student_list)):
            item = QStandardItem(data.student_list[row].id)
            self.model.setItem(row, 0, item)
            item = QStandardItem(data.student_list[row].name)
            self.model.setItem(row, 1, item)
            item = QStandardItem(str(data.student_list[row].score))
